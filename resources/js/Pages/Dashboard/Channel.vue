...
const metrics = [
    ...
    { label: 'Avg Views',   value: formatNumber(props.channel.avg_views),   icon: 'eye', color: 'emerald' },
...
<template>
    <DashboardLayout>
        ...
        <div class="flex items-center justify-between mb-8">
            <Link href="/dashboard" class="group flex items-center gap-2 text-xs font-black uppercase tracking-[0.2em] text-gray-500 hover:text-white transition-all">
            ...
            <div class="flex items-center gap-2">
                 <span v-if="channel.is_active" class="flex h-2 w-2 rounded-full bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.8)]"></span>
                 <p class="text-[10px] font-black uppercase tracking-widest text-gray-500">Live Intel</p>
            </div>
        </div>

        <!-- Premium Hero Strip -->
        <div class="bg-[#111118] border border-white/[0.05] rounded-[2.5rem] p-10 mb-8 border-b-4 border-b-indigo-500/20">
            <div class="flex flex-col lg:flex-row lg:items-center gap-10">
                ...
                <div class="flex items-center gap-6 pr-10 lg:border-r border-white/5 min-w-[300px]">
                    <div class="w-20 h-20 rounded-3xl bg-gradient-to-br from-indigo-500/20 to-purple-500/20 border border-white/5 flex items-center justify-center text-3xl font-black text-white shadow-inner">
                ...
                <!-- Horizontal metrics -->
                <div class="flex-1 grid grid-cols-2 xl:grid-cols-4 gap-8">
                    <div v-for="m in metrics" :key="m.label" class="flex items-center gap-4 group">
                        <div :class="`w-10 h-10 rounded-xl bg-${m.color}-500/10 flex items-center justify-center text-${m.color}-400 group-hover:scale-110 transition-transform`">
                             <svg v-if="m.icon === 'users'" class="w-5 h-5 text-indigo-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"/></svg>
                             <svg v-if="m.icon === 'eye'" class="w-5 h-5 text-emerald-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                             <svg v-if="m.icon === 'trending-up'" class="w-5 h-5 text-purple-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6"/></svg>
                             <svg v-if="m.icon === 'zap'" class="w-5 h-5 text-amber-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"/></svg>
                        </div>
                        <div>
            ...
            <div class="lg:col-span-2 space-y-8">
                ...
                <div class="bg-[#111118] border border-white/[0.05] rounded-[2.5rem] p-8">
                    <div class="flex items-center justify-between mb-10 px-4">
                        <div>
                            ...
                            <p class="text-xs font-bold text-gray-600 mt-1 uppercase tracking-widest">Growth metrics over time</p>
                        </div>
                ...
                <div class="bg-[#111118] border border-white/[0.05] rounded-[2.5rem] overflow-hidden">
                    <div class="px-10 py-8 border-b border-white/[0.03] flex items-center justify-between">
                        ...
                        <span class="text-[10px] font-black uppercase tracking-[0.2em] text-gray-700">{{ posts.length }} Events Tracked</span>
                    </div>

                    <div v-if="posts.length > 0" class="divide-y divide-white/[0.02]">
                        <div v-for="post in posts" :key="post.id" class="px-10 py-6 hover:bg-white/[0.01] transition-all group">
                            <div class="flex items-center justify-between gap-10">
                                <div class="flex-1 min-w-0">
                                    <div class="flex items-center gap-3 mb-2">
                                        <div class="px-2 py-0.5 rounded-md bg-white/5 border border-white/5 text-[9px] font-black uppercase tracking-widest text-gray-500 border border-white/10">
                                            {{ post.media_type || 'POST' }}
                                        </div>
                                        <p class="text-[10px] font-bold text-gray-600 uppercase tracking-widest">{{ post.posted_at_ago }}</p>
                                    </div>
                                ...
                                <div class="flex items-center gap-10 shrink-0 pr-4">
                                    <div class="text-right">
                                        ...
                                        <p class="text-[9px] font-bold text-gray-700 uppercase tracking-widest">Views</p>
                                    </div>
                                    <div class="text-right min-w-[60px]">
                                        <p class="text-sm font-black text-red-500">{{ formatNumber(post.reactions) }}</p>
                 ...
                 <div class="bg-[#111118] border border-white/[0.05] rounded-[2.5rem] p-8">
                    ...
                    <div class="space-y-6">
                        <div class="flex items-center justify-between">
                            ...
                            <span class="px-2.5 py-1 rounded-lg bg-emerald-500/10 text-emerald-500 text-[10px] font-black uppercase tracking-widest border border-emerald-500/10">Tracking</span>
                        </div>
                        <div class="flex items-center justify-between">
                            <span class="text-[10px] font-bold text-gray-600 uppercase tracking-widest">Network Score</span>
                            <span class="text-xs font-black text-white">{{ channel.potential_score }}%</span>
                        </div>
                        <div class="flex items-center justify-between">
                            <span class="text-[10px] font-bold text-gray-600 uppercase tracking-widest">Last Intel Sync</span>
                            <span class="text-[10px] font-black text-gray-400">{{ channel.last_synced_at }}</span>
                        </div>
                    </div>

                    <div class="mt-10 pt-8 border-t border-white/[0.03]">
                         <button class="w-full py-4 rounded-2xl bg-white/5 border border-white/5 text-[10px] font-black uppercase tracking-widest text-gray-400 hover:text-white hover:bg-white/10 transition-all cursor-not-allowed">
