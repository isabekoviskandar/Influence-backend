    ...
    <DashboardLayout>
        <div class="max-w-5xl mx-auto py-8">
            ...
            <div class="text-center mb-12">
                ...
                <p class="text-[#8a8a8f] max-w-lg mx-auto">Get extended analytics, connect more channels, and unlock deeper insights.</p>
            </div>

            <!-- Payment Notice Banner -->
            <div class="mb-14 p-6 bg-[#6366f1]/10 border border-[#6366f1]/30 rounded-xl max-w-3xl mx-auto flex flex-col md:flex-row items-center gap-6">
                <div class="flex-shrink-0 w-12 h-12 bg-[#6366f1]/20 rounded-full flex items-center justify-center">
                    <svg class="w-6 h-6 text-[#6366f1]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                </div>
                <div class="flex-1 text-center md:text-left">
                    ...
                    <p class="text-[#8a8a8f] text-sm leading-relaxed mb-4">
                        We are currently integrating a secure online payment gateway. For now, if you'd like to upgrade to Pro or Premium, please process your payment directly with the admin via Telegram.
                    </p>
            ...
            <div class="grid md:grid-cols-3 gap-6">
                <div v-for="plan in plans" :key="plan.name" 
                    ...
                    <div v-if="plan.popular" class="absolute top-0 right-8 -translate-y-1/2 bg-[#6366f1] text-white text-[10px] uppercase tracking-wider font-bold px-3 py-1 rounded-full">
                        MOST POPULAR
                    </div>
                    ...
                    <ul class="space-y-4 mb-10 flex-1">
                        <li v-for="feature in plan.features" :key="feature" class="flex items-start gap-3">
                            <svg class="w-5 h-5 flex-shrink-0 mt-0.5" :class="plan.color" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg>
                            <span class="text-[#a1a1aa] text-[14px]">{{ feature }}</span>
                        </li>
                    </ul>

                    <div v-if="plan.current" class="w-full py-2.5 rounded-lg borde r border-[#222224] bg-[#222224]/50 text-[#8a8a8f] text-center text-sm font-medium cursor-default">
                        Current Plan
                    </div>
